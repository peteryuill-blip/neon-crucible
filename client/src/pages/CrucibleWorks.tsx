...
export default function CrucibleWorks() {
  ...
  const rawData = JSON.stringify(data, null, 2).slice(0, 2000);
  const errorMsg = error ? String(error.message) : null;
    ...
    <div className="max-w-7xl mx-auto py-12 px-4">
      ...
      <div className="space-y-6 mb-12">
        ...
        <p className="font-serif text-base sm:text-lg leading-relaxed text-foreground/85 max-w-3xl">
          A complete index of the Crucible phase, ordered by critical weight.
        </p>
      ...
      <div className="mb-8 border border-red-500/30 rounded-sm p-4 bg-red-950/20">
        ...
        {isLoading && <p className="font-mono text-xs text-yellow-400">Loading...</p>}
        {errorMsg && <p className="font-mono text-xs text-red-400">Error: {errorMsg}</p>}
        {!isLoading && !error && (
          <>
            <p className="font-mono text-xs text-green-400">Total items fetched: {Array.isArray(data) ? data.length : 0}</p>
            <p className="font-mono text-xs text-green-400">Filtered (phaseId=60606): {works.length}</p>
            <p className="font-mono text-xs text-muted-foreground mt-2">Raw data sample:</p>
            <pre className="font-mono text-[10px] text-muted-foreground overflow-x-auto mt-1">
              {rawData}
            </pre>
          </>
        )}
